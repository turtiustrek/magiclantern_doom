...
void D_DoomLoop(void)
{
    uart_printf("D_DoomLoop!\n");
    if (bfgedition &&
        (demorecording || (gameaction == ga_playdemo) || netgame))
    {
        uart_printf(" WARNING: You are playing using one of the Doom Classic\n"
...
static char *GetGameName(char *gamename)
{
    ...
    for (i = 0; i < arrlen(banners); ++i)
    {
        ...
        if (deh_sub != banners[i])
        {
            ...
            // Has been replaced.
            // We need to expand via uart_printf to include the Doom version number
...
static void SetMissionForPackName(char *pack_name)
{
    ...
    uart_printf("Valid mission packs are:\n");
...
static void InitGameVersion(void)
{
    ...
    if (p)
    {
        ...
        {
            uart_printf("Supported game versions:\n");
...
void PrintGameVersion(void)
{
    ...
    for (i = 0; gameversions[i].description != NULL; ++i)
    {
        ...
        {
            uart_printf("Emulating the behavior of the "
                        "'%s' executable.\n",
                        gameversions[i].description);
            break;
        }
...
static void LoadIwadDeh(void)
{
    ...
    if (gameversion == exe_chex)
    {
        ...
        if (sep != NULL)
        {
            ...
            chex_deh = _AllocateMemory(chex_deh_len);
...
void D_DoomMain(void)
{
    ...
    {
        uart_printf("Dedicated server mode.\n");
        NET_DedicatedServer();
    ...
    M_SetConfigFilenames("/DEFAULT.CFG", PROGRAM_PREFIX "/DEFAULT.CFG");
    DEH_printf("D_BindVariables\n");
    D_BindVariables();
    DEH_printf("M_LoadDefaults\n");
    M_LoadDefaults();

    // Save configuration at exit.
    I_AtExit(M_SaveDefaults, false);
    DEH_printf("D_FindIWAD\n");
...
#endif
    DEH_printf("W_CheckCorrectIWAD\n");
    W_CheckCorrectIWAD(doom);

    // Now that we've loaded the IWAD, we can figure out what gamemission
    // we're playing and which version of Vanilla Doom we need to emulate.
    DEH_printf("D_IdentifyVersion\n");
    D_IdentifyVersion();
    DEH_printf("InitGameVersion\n");
    InitGameVersion();
    ...
    DEH_printf("W_CheckNumForName\n");
    if (W_CheckNumForName("dmenupic") >= 0)
    {
        uart_printf("BFG Edition: Using workarounds as needed.\n");
        bfgedition = true;
    ...
    if (p)
    {
        ...
        uart_printf("Playing demo %s.\n", file);
    }
    uart_printf("I_AtExit\n");
    I_AtExit((atexit_func_t)G_CheckDemoStatus, true);

    // Generate the WAD hash table.  Speed things up a bit.
    uart_printf("W_GenerateHashTable\n");
    W_GenerateHashTable();
    ...
    if (M_ParmExists("-dehlump"))
    {
        ...
        uart_printf("  loaded %d DEHACKED lumps from PWAD files.\n", loaded);
    }
#endif

    // Set the gamedescription string. This is only possible now that
    // we've finished loading Dehacked patches.
    uart_printf("D_SetGameDescription\n");
    ...
    {
        I_PrintDivider();
        uart_printf(" WARNING: The loaded WAD file contains modified sprites or\n"
    ...
    {
        uart_printf(" WARNING: You are playing using one of the Freedoom IWAD\n"
...
#ifdef FEATURE_MULTIPLAYER
    uart_printf("NET_Init: Init network subsystem.\n");
    NET_Init();
#endif
